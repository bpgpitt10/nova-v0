#!/usr/bin/env python3
"""Passive GSPro source/timing probe v2 for Looper live-state research.

This probe intentionally presses no keys and changes no GSPro state. It records the
structured sources Looper can observe locally and independently samples the existing
screen readers so source timing/authority can be proven from one timeline.

V2 audit changes:
- saves currentRound bytes even when a read catches GSPro mid-write;
- correlates GSPro.db Round rows to currentRound.RoundID when possible instead of
  assuming the highest database ID is always the active round;
- snapshots the complete SQLite table/column schema for later research;
- preserves exact RoundSettings/RoundData bytes when they change instead of keeping
  only hashes;
- excludes noisy raw OCR strings from screen change detection so an overnight probe
  cannot create hundreds of screenshots just because Tesseract jittered;
- records processing latency separately from event timestamps.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Passive GSPro structured-source/timing probe v2")
...
def _latest_shot(summaries: list[dict[str, Any]]) -> dict[str, Any] | None:
    if not summaries:
        return None
    with_numbers = [row for row in summaries if isinstance(row.get("global_shot_number"), int)]
    if with_numbers:
        return max(with_numbers, key=lambda row: int(row["global_shot_number"]))
    return summaries[-1]


class SharedProbeState:
    """Tiny cross-source bridge used only to correlate database rows during research."""
    ...
    def update_from_current_round(self, latest: dict[str, Any] | None) -> None:
        ...
        with self._lock:
            try:
                ...
                pass
            course = latest.get("course_key")
            if course:
                self._course_key = str(course)
            hole = latest.get("hole_display")
            if isinstance(hole, int):
                self._last_completed_hole = hole
...
class EventWriter:
    def __init__(self, output_root: Path) -> None:
        ...
        self.session_dir = output_root / f"gspro_event_probe_v2_{stamp}"
    ...
    def save_db_artifact(self, round_id: Any, field: str, raw: bytes) -> str:
        with self._lock:
            ...
            name = f"{self._db_artifact_snapshot:04d}_round-{safe_round}_{safe_field}.bin"
            (self.db_artifact_dir / name).write_bytes(raw)
            return name

    def save_db_schema(self, schema: dict[str, Any]) -> None:
        path = self.session_dir / "gspro_db_schema.json"
...
def _sqlite_schema(connection: sqlite3.Connection) -> dict[str, Any]:
...
def _read_round_context(path: Path, preferred_round_id: int | None) -> tuple[dict[str, Any] | None, str, list[dict[str, Any]], dict[str, Any]]:
    connection = sqlite3.connect(str(path), timeout=0.08)
    try:
        connection.row_factory = sqlite3.Row
        connection.execute("PRAGMA query_only=ON")
        schema = _sqlite_schema(connection)
        columns = [str(row[1]) for row in connection.execute('PRAGMA table_info("Round")').fetchall()]
        if not columns:
            return None, "round-table-missing", [], schema
        ...
        if not selected:
            return None, "round-columns-missing", [], schema
        ...
        if preferred_round_id is not None and "ID" in columns:
            row = connection.execute(
                f'SELECT {quoted} FROM "Round" WHERE "ID"=? LIMIT 1',
                (preferred_round_id,),
            ).fetchone()
            if row is not None:
                method = "currentRound.RoundID"
        if row is None:
            order = ' ORDER BY "ID" DESC' if "ID" in columns else ""
            row = connection.execute(f'SELECT {quoted} FROM "Round"{order} LIMIT 1').fetchone()

        recent_fields = [name for name in (
            "ID", "PlayerName", "CourseCode", "CourseName", "ActiveHole",
            "RoundStatus", "RoundType", "DateModified",
        ) if name in columns]
        recent: list[dict[str, Any]] = []
        if recent_fields:
            recent_quoted = ",".join(f'"{name}"' for name in recent_fields)
            order = ' ORDER BY "ID" DESC' if "ID" in columns else ""
            recent_rows = connection.execute(f'SELECT {recent_quoted} FROM "Round"{order} LIMIT 5').fetchall()
            recent = [dict(item) for item in recent_rows]

        if row is None:
            return None, method, recent, schema
        ...
        payload["_artifacts"] = artifacts
        return normalize_round_db_row(payload), method, recent, schema
    finally:
        connection.close()
...
def _db_worker(
    ...
    last_artifact_hashes: dict[tuple[Any, str], str] = {}
    last_error: str | None = None
    ...
    while not stop.is_set():
        try:
            ...
            preferred_round_id = shared_snapshot.get("current_round_id")
            row, selection_method, recent, schema = _read_round_context(path, preferred_round_id)

            current_schema_hash = _json_hash(schema)
            if current_schema_hash != schema_hash:
                writer.save_db_schema(schema)
                writer.emit("gspro_db", "schema_snapshot", {
                    "sha256": current_schema_hash,
                    "object_count": len(schema.get("objects") or []),
                })
                schema_hash = current_schema_hash

            artifacts = dict((row or {}).pop("_artifacts", {}) if row is not None else {})
            ...
            for field, raw in artifacts.items():
                ...
                if last_artifact_hashes.get(key) != digest:
                    artifact_files[field] = writer.save_db_artifact(row_id, field, raw)
            ...
            if signature != last_signature:
                ...
                payload = {
                    **signature_payload,
                    ...
                    "new_artifact_files": artifact_files,
                }
                console = None
                if row is not None:
                    console = (
                        f"GSPro.db {event}: round={row.get('ID')} via {selection_method} | "
                        f"ActiveHole raw={row.get('ActiveHoleRaw')} "
                        f"display~={row.get('ActiveHoleDisplayAssumingZeroBased')} | "
                        f"status={row.get('RoundStatus')}"
                    )
                writer.emit("gspro_db", event, payload, console)
...
def _screen_semantic_signature(
    surface: dict[str, Any] | None,
    upper: dict[str, Any] | None,
    lie: dict[str, Any] | None,
    identity: dict[str, Any] | None,
) -> dict[str, Any]:
    """Use normalized facts for change detection; never raw OCR strings."""
    return {
        "surface": {
            "label": (surface or {}).get("label"),
            "is_tee": (surface or {}).get("is_tee"),
            "recognized": (surface or {}).get("recognized"),
        },
        "upper_left": {
            "player": (upper or {}).get("player_name"),
            "shot": (upper or {}).get("shot_number"),
            "dtp": (upper or {}).get("distance_to_pin_yds"),
            "elevation": (upper or {}).get("elevation_delta_yds"),
        },
        "lie": {
            "up_down": (lie or {}).get("signed_up_down_deg"),
            "left_right": (lie or {}).get("signed_left_right_deg"),
        },
        "identity": {
            "course": (identity or {}).get("course_name"),
            "hole": (identity or {}).get("hole_number"),
            "par": (identity or {}).get("par"),
            "yards": (identity or {}).get("hole_yards"),
        },
    }
    ...
    with ThreadPoolExecutor(max_workers=4, thread_name_prefix="gspro-screen-probe") as executor:
        while not stop.is_set():
            ...
            try:
                ...
                semantic = _screen_semantic_signature(surface_dict, upper_dict, lie_dict, identity_dict)
                signature = _json_hash(semantic)
...
def main() -> int:
    ...
    manifest = {
        "schema_version": "looper-gspro-event-probe-v2",
        ...
        "research_questions": [
            "Does GSPro.db Round.ActiveHole change before the next tee shot is hit?",
            "Which output_log material values appear for fairway/rough/sand/green/penalty states?",
            "What is the timing order among ShotCompleted, AllPlayersHoledOut, ActiveHole and Tee screen state?",
            "Do any output_log lines expose usable wind speed/direction rather than only diagnostics?",
    ...
    print("Looper GSPro PASSIVE EVENT PROBE v2", flush=True)
    ...
    threads = [
        threading.Thread(
            ...
            args=(current_round, writer, shared, stop, max(0.02, args.file_poll_ms / 1000.0)),
            daemon=True,
        ...
        threading.Thread(
            target=_db_worker,
            args=(db, writer, shared, stop, max(0.04, args.db_poll_ms / 1000.0)),
            daemon=True,
            name="gspro-db-watch",
        ),
    ]
