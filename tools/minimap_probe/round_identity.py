...
The full-screen GSPro HUD carries a stable course/hole block in the upper-right:
...
def read_round_identity(
    ...
    if debug_dir is not None:
        ...
        x, y, bw, bh = header_box
        cv2.rectangle(annotated, (x, y), (x + bw, y + bh), (0, 255, 255), 2)
        label = f"{course or '?'} | H{hole or '?'} | P{par or '?'} | {yards or '?'}y | {confidence:.2f}"
        cv2.putText(annotated, label, (x, max(25, y - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 255, 255), 2)
        cv2.imwrite(str(out / "round_identity_debug.png"), annotated)
