#!/usr/bin/env python3
"""Offline/read-only GSPro water probe v0.

Runs on a saved tee capture. It does not focus GSPro, press keys, alter zoom,
or mutate the canonical HoleModel. It writes review artifacts and a preview model
only, so water thresholds can be tuned safely from real course captures.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Offline GSPro water extractor v0")
    p.add_argument("--capture-dir", help="tee_capture_* directory containing hole_model.json and a saved minimap")
    ...
    p.add_argument("--min-confidence", type=float, default=0.48)
...
def _latest_capture(output_root: Path) -> Path:
    candidates = [p for p in output_root.glob("tee_capture_*") if p.is_dir() and (p / "hole_model.json").exists()]
    if not candidates:
...
def _resolve_inputs(args: argparse.Namespace) -> tuple[Path, Path, Path]:
    ...
    if args.image:
        ...
        # Use restored normal visual colors, not the Y heatmap frame.
...
def _preview_hole_model(model: dict, result, image_path: Path) -> dict:
    ...
    hazards["water_objects"] = [asdict(obj) for obj in result.objects]
    hazards["water_source"] = "visual-segmentation:cool-water-v0"
    hazards["water_validation_state"] = "offline-preview-unvalidated"
    hazards["water_source_image"] = image_path.name
    return preview
...
def main() -> int:
    args = parse_args()
    if args.self_test:
        result = water_extractor.synthetic_self_test()
    ...
    try:
        ...
        mask_path = out / "water_mask_v0.png"
        candidates_path = out / "water_candidates_v0.png"
        overlay_path = out / "water_debug_overlay_v0.png"
        json_path = out / "water_v0.json"
        preview_path = out / "hole_model_water_preview_v0.json"

        cv2.imwrite(str(mask_path), result.mask)
        cv2.imwrite(str(candidates_path), result.candidate_mask)
        cv2.imwrite(str(overlay_path), water_extractor.draw_debug_overlay(image, result, ball_xy=ball, pin_xy=pin))
        ...
        preview_path.write_text(json.dumps(_preview_hole_model(model, result, image_path), indent=2), encoding="utf-8")
        ...
        print()
        print("GSPro WATER IDENTIFICATION PROBE v0")
        print("===================================")
        ...
        print(f"Accepted water bodies: {result.accepted_count}")
        if not result.objects:
            print("  None accepted. Review candidate mask/diagnostics before changing thresholds.")
        for obj in result.objects:
            ...
            print(
                f"  W{obj.object_id}: conf {obj.confidence:.2f}; "
        ...
        print(f"Preview HoleModel:     {preview_path}")
        print()
        print("Important: red penalty boundaries remain authoritative for penalty semantics; water is a separate visual terrain layer.")
        return 0
