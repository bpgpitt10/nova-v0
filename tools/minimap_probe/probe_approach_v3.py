#!/usr/bin/env python3
"""GSPro post-tee v3: authoritative hole identity + pin-optional canonical geometry.

Field-safe behavior remains unchanged: no W, no Y, no recommendation-driven AIM.
v3 adds the persistent upper-right GSPro course/hole header as the HoleModel cache
key and removes the requirement that the white minimap pin be visible after the tee.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="GSPro post-tee ShotState probe v3")
...
def main() -> int:
    ...
    try:
        ...
        # PIN, lie and course/hole identity are all frozen-screen reads.  Hide their
        # OCR cost under the unavoidable neutral AIM acquisition sequence.
        with ThreadPoolExecutor(max_workers=3, thread_name_prefix="approach-v3") as executor:
        ...
        payload = {
            "schema_version": "post-tee-shot-state-v3",
            ...
            "wind": None,
            "wind_note": "Use existing Looper wind source; v3 accepts its derived carry/lateral adjustments.",
        ...
        print()
        print("GSPro POST-TEE SHOT STATE PROBE v3")
        print("=================================")
        ...
        if identity_payload:
            ...
            print("Hole identity:          unavailable")
        if identity_warning:
            print(f"Identity warning:       {identity_warning}")
        selection = geometry.get("hole_model_selection") or {}
        print(
            f"HoleModel selection:    {selection.get('method', '?')} | "
            f"confidence {float(selection.get('confidence') or 0.0):.2f}"
        )
        ...
        print(
            f"Minimap pin visible:    {'YES' if markers.get('pin_marker_visible') else 'NO'} | "
            "not required"
        )
        if markers.get("pin_marker_visible") and markers.get("pin_projection_error_px") is not None:
            print(
                f"Pin projection check:   {float(markers['pin_projection_error_px']):.1f} px | "
                f"{'PASS' if markers.get('pin_marker_verified_against_projection') else 'WARN'}"
            )
        ...
        green_state = geometry["green_visibility"]
        print(f"Target green visible:  {'YES' if green_state['visible'] else 'NO'}")
