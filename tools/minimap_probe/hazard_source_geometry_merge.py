...
def normalize_source(path: Path, identity: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    payload = read_json(path)
    hint = "gkd" if path.name.lower() == "features.json" else "unity" if "geometry" in path.name.lower() else None
    return hg.normalize_payload(payload, artifact=str(path), source_hint=hint, identity=identity)
...
def merge_capture(capture: Path, source_paths: list[Path], identity: dict[str, Any]) -> dict[str, Any]:
    ...
    for path in source_paths:
        rows, problems = normalize_source(path, identity)
    ...
    if manifest_path.is_file():
        ...
        manifest.setdefault("source_status", {})["fresh_course_archaeology"] = {
            ...
            "note": "Course-level identity only; no per-hole/capture transform was invented.",
...
def main() -> int:
    ...
    for capture in captures:
        try:
            ...
            print(f"PASS {capture.name} | source_added={row['source_objects_added']} | bundle={row['bundle_object_count']} | {row['source_counts']}")
