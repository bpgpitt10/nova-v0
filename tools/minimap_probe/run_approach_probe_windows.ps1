...
$argsList = @(
  (Join-Path $Here "probe_approach_v0.py"),
  "--monitor", "$Monitor",
...
Write-Host "Running GSPro POST-TEE ShotState probe v0 once."
Write-Host "No W zoom. No Y heatmap. Cached tee HoleModel remains untouched."
Write-Host "Reads PIN distance/elevation + REAL lie slope and acquires AIM with neutral arrow return."
Write-Host "PIN/lie OCR overlap AIM acquisition for speed."
