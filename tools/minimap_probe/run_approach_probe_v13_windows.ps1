...
$argsList = @(
  (Join-Path $Here "probe_approach_v13.py"),
  "--monitor", "$Monitor",
...
Write-Host "Running GSPro POST-TEE ShotState probe v1.3 once."
Write-Host "AIM policy: passive minimap marker first; bounded AIM-card summon only if marker is unavailable."
Write-Host "FAIL-SOFT: PIN/lie/AIM/green failures become unavailable sensors, not discarded ShotStates."
Write-Host "SAFE: no W zoom and no Y heatmap. Passive path sends NO GSPro input."
& $Python @argsList
exit $LASTEXITCODE
