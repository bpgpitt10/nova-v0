...
CALCULATIONS = {
    ...
    "shot_mode_inference": CalculationSpec(
        ...
        ("shot_mode.non_full_shot_surfaces", "shot_mode.non_full_shot_surface_confidence",
         ...
         "shot_mode.aim_pin_distance_absolute_tolerance_yds", "shot_mode.aim_pin_distance_relative_tolerance_fraction",
    ...
    "candidate_generation": CalculationSpec(
        ...
        "Create Stock, synthetic Smooth and explicit playable shot candidates around the correct spatial target vector without redefining Looper Stock/Pure; explicit variants are model candidates only when they own their observed pattern statistics and are model-ready.",
        "tools.live_caddie.candidates.generate_candidates",
        ("ClubProfile[] including explicit variant model_ready/pattern fields", "LiveShotState"),
        ("CandidateShot[]",),
    ...
    "short_game_shot_coverage": CalculationSpec(
        "short-game-shot-coverage", "v1",
        "Determine whether an approach is supported by the complete model-ready Stock/Smooth/explicit shot inventory, should fall back to geometry-only safe-target guidance, or is outside the configured guidance scope.",
        ...
        ("short_game.modeled_coverage_absolute_tolerance_yds",
         ...
         "short_game.maximum_geometry_guidance_distance_yds"),
        ("candidate-generation", "effective-target-distance"),
    ),
    "short_game_geometry_guidance": CalculationSpec(
        "short-game-geometry-guidance", "v1",
        "When no modeled partial shot exists, search known-safe green geometry in both lateral and short/long directions and use penalty-boundary clearance only inside that known-safe surface; never invent club, spin, dispersion or probability.",
        ...
        ("ShortGameGuidance",),
        ("short_game.geometry_lateral_offsets_yds", "short_game.geometry_longitudinal_offsets_yds",
         "short_game.green_clearance_reference_yds", "short_game.hazard_clearance_reference_yds",
         "short_game.aim_offset_reference_yds", "short_game.minimum_side_advantage_score",
         "short_game.geometry_scoring", "short_game.confidence_green_and_hazard",
         "short_game.confidence_green_only", "short_game.confidence_hazard_only",
         "short_game.confidence_no_geometry", "short_game.unknown_green_confidence_multiplier"),
