 ...
 *
 * Raw shots are never mutated. The standalone classifier remains the source of
 * derived mishit interpretation; this module only groups Looper shots by
 * club + variant, runs the classifier, and exposes one planning-eligibility
 * boundary for downstream calculations.
 */
...
export const mishitPlanningPopulationKey = (shot: Pick<Shot, 'club' | 'shotVariantId'>) =>
...
export const buildMishitPlanningState = (
  ...
  playerCalibrations: Record<string, MishitPlayerCalibration> =
    loadMishitPlayerCalibrations(),
): MishitPlanningState => {
  const groupedShots = new Map<string, Shot[]>()

  sessions.forEach((session) => {
    session.shots.forEach((shot) => {
      // Manual/source exclusion remains separate and stronger than derived
  ...
  groupedShots.forEach((shots, populationKey) => {
    const analysis = analyzeShotPopulation(
      ...
      DEFAULT_MISHIT_CONFIG,
      playerCalibrations[populationKey],
    )
...
export const installMishitPlanningState = (
  sessions: SavedSession[],
  playerCalibrations?: Record<string, MishitPlayerCalibration>,
) => {
  installedPlanningState = buildMishitPlanningState(
    sessions,
    playerCalibrations ?? loadMishitPlayerCalibrations(),
  )
